---
title: "<span style=\"color: blue; text-decoration: none;\"> 轻声恢复 </span>"     
collection: talks
type: "Talk"
permalink: /talks/whisper
---

---
- <font size=3> 轻声语音（Whisper）是一类常见于会议等实际应用场景中的特殊语音形态，通常表现为说话音量较低、能量分布较弱，并伴随部分频率成分缺失。由于其语音信息存在天然的不完整性，将轻声语音恢复为自然清晰的正常语音仍是一项具有挑战性的任务。</font>  


---
###  <font size=4> 恢复效果对比</font>
-- <font size=3> 利用语义特征，设计了一个基于生成式对抗恢复网络，区别于常用语音增强方案。</font>  
  
-- <font size=3> 轻声</font>  
![](/images/whisperbefore.JPG){: .align-center width="700px"}
​<audio id="audio" controls="" preload="none">
      <source id="wav" src="../files/whisperbefore.wav">


-- <font size=3> 恢复之后</font>  
![Whisper after](/images/whisperafter.JPG){: .align-center width="700px"}
 ​<audio id="audio" controls="" preload="none">
      <source id="wav" src="../files/whisperafter.wav">

-- <font size=3> 正常语音</font>  
![normal](/images/normal.JPG){: .align-center width="700px"}
 ​<audio id="audio" controls="" preload="none">
      <source id="wav" src="../files/normal.wav">

  
 
 

